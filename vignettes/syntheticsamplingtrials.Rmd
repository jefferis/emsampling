...
Now, in both cases we see that m=20 partners is our maximum likelihood estimate. 
...
are from cells of the type, carrying the same information, then such weak inputs
would be more likely to have an impact. This means that deciding on a threshold
connection strength for "interesting" partner neurons may be difficult.


It might be interesting to consider a family of distributions that could be parameterised to 
show different amounts of skew.
